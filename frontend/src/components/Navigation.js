      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Tenant Badge - Only for regular tenant users, NOT super admin */}
            {!isSuperAdmin() && tenant && tenant.code !== 'TASKACT1' && (
              <>
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    <div className="hidden lg:block text-left">
                      ...
                      <div className={`text-xs capitalize ${isDarkTheme ? 'text-slate-400' : 'text-gray-500'}`}>{user?.role?.replace('_', ' ')}</div>
                    </div>
                    <ChevronDown className={`h-4 w-4 hidden lg:block ${isDarkTheme ? 'text-slate-400' : 'text-gray-400'}`} />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className={`w-56 ${isDarkTheme ? 'bg-slate-800 border-slate-700' : ''}`}>
                  <div className={`px-3 py-2 border-b ${isDarkTheme ? 'border-slate-700' : 'border-gray-100'}`}>
                    <div className={`text-sm font-medium ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>{user?.name}</div>
                    <div className={`text-xs ${isDarkTheme ? 'text-slate-400' : 'text-gray-500'}`}>{user?.email}</div>
                  </div>
                  
                  <DropdownMenuItem 
                    onClick={() => setShowChangePassword(true)}
                    className={`cursor-pointer ${isDarkTheme ? 'text-slate-300 focus:bg-slate-700 focus:text-white' : ''}`}
                  >
                  ...
                  {isSuperAdmin() && (
                    <>
                      <DropdownMenuSeparator className={isDarkTheme ? 'bg-slate-700' : ''} />
                      <DropdownMenuItem asChild className={isDarkTheme ? 'text-slate-300 focus:bg-slate-700 focus:text-white' : ''}>
          ...
          <div className="md:hidden flex items-center space-x-2">
            <NotificationPanel />
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isDarkTheme ? 'bg-indigo-600' : 'bg-blue-100'}`}>
              <span className={`font-medium text-sm ${isDarkTheme ? 'text-white' : 'text-blue-600'}`}>
                {user?.name?.charAt(0)?.toUpperCase()}
              </span>
            </div>
            <button
              ...
              className={`inline-flex items-center justify-center p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-inset ${
                isDarkTheme 
                  ? 'text-slate-300 hover:text-white hover:bg-slate-700 focus:ring-indigo-500' 
